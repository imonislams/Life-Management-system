...
@endphp
<x-app-layout>
    <x-slot name="title">Add Activity - Daily Activities</x-slot>
    <x-slot name="pageTitle">Daily Activities</x-slot>

    <div class="card" style="max-width: 680px; margin: 0 auto;">
        <h1 class="card-title">Record an Activity</h1>
        <p class="card-subtitle" style="margin-bottom: 1.5rem;">Write down what you actually did today.</p>
        ...
        <form method="POST" action="{{ route('daily-activities.store') }}">
            ...
            <div class="form-group">
                ...
                <input id="title" type="text" name="title" value="{{ old('title') }}" required autofocus class="form-control" placeholder="e.g. Studied Laravel for 2 hours">
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" rows="3" class="form-control" placeholder="What did you do? What did you learn or complete?">{{ old('description') }}</textarea>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                <div class="form-group">
                    ...
                    <input id="activity_date" type="date" name="activity_date" value="{{ old('activity_date', $presetDate ?: now()->toDateString()) }}" required class="form-control">
                    @error('activity_date')<div class="error-msg">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="category" class="form-label">Category (Optional)</label>
                    <input id="category" type="text" name="category" value="{{ old('category') }}" class="form-control" placeholder="e.g. Study, Work, Health, Personal">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1rem;">
                <div class="form-group">
                    ...
                    <input id="start_time" type="time" name="start_time" value="{{ old('start_time') }}" class="form-control">
                ...
                <div class="form-group">
                    ...
                    <input id="duration_minutes" type="number" min="0" name="duration_minutes" value="{{ old('duration_minutes') }}" class="form-control" placeholder="Auto from times">
                    <div style="font-size:0.72rem; color:#94a3b8;">Calculated automatically from start/end times.</div>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                <div class="form-group">
                    ...
                    <select id="goal_id" name="goal_id" class="form-control">
                        ...
                        <option value="{{ $goalOption->id }}" {{ (string) old('goal_id') === (string) $goalOption->id ? 'selected' : '' }}>{{ $goalOption->title }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control">
                        ...
                        <option value="{{ $status }}" {{ old('status', 'completed') === $status ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $status)) }}</option>
                        @endforeach
                    </select>
